...
export function generateTaskList(preCommands: CommandSpec[], targetDir: string, session: any) {
  const preCommandsInfo = preCommands.map((object) => dynamapping(object, session, {})).filter((x) => !x.prevent);
  ...
  return preCommandsInfo.map((commandSpec: CommandSpec) => {
    return {
      ...
      task: async () => {
        await execa(
          commandSpec.file,
          convertCommandArgs(commandSpec.arguments, targetDir, session),
          convertCommandOptions(commandSpec.options, targetDir, session),
        ).catch((error: any) => {
